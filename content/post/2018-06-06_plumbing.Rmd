...
The goal is to end up with a 'REST' service, which allows you to browse to an URL like this,
...
This makes it very easy for your system admin to deploy this particular API on any port they wish, instead of poking around in our code.
...
Obviously this was no introduction to `plumber`, just a note on some settings I collected during my use of the package. The neat organization of the API definitions, and the code to deploy the API, together with `optparse` for command line options makes for a powerful (and easy to use) service to deploy a REST API. 
