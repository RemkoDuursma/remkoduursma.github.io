...
group_by(cycsyd_a, Brand2) %>%
  filter(n() > 25) %>% ungroup %>%
  mutate(Brand = reorder(Brand2, price_low_median, 
                         function(x)-median(x, na.rm=TRUE))) %>%
  ggplot(aes(x=Brand, y=price_low_median, fill=Brand2)) +
  geom_boxplot() + 
  theme_tufte() +
  theme(legend.position="none") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
group_by(cycsyd_a, Brand2) %>%
  filter(n() > 25) %>% ungroup %>%
  mutate(Brand2 = reorder(Brand2, price_peak_median, 
                         function(x)-median(x, na.rm=TRUE))) %>%
  ggplot(aes(x=Brand2, y=price_peak_median, fill=Brand2)) +
  geom_boxplot() + 
  theme_tufte() +
  theme(legend.position="none") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
